...
import { SlidingWindowBudgetImpl } from '../../src/again/again.budget.js';
import type { AgainBudgetConfig } from '../../src/again/types.js';

const defaultConfig: AgainBudgetConfig = {
  windowMs: 10_000,
  maxRetryRatio: 0.2,
  minRequestCount: 5,
};

function makeBudget(overrides?: Partial<AgainBudgetConfig>): SlidingWindowBudgetImpl {
...
describe('SlidingWindowBudgetImpl', () => {
  ...
  it('resets counts after window expires', () => {
    ...
    // New calls in fresh window — below minRequestCount again
    budget.recordCall();
    expect(budget.tryConsume()).toBe(true);
    vi.useRealTimers();
  });
