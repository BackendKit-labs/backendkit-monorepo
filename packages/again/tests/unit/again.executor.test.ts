...
import { AgainExecutor, type ExecutorDependencies } from '../../src/again/again.executor.js';
import { HookRunner } from '../../src/again/again.hooks.js';
...
import { defaultAgainCondition, defaultAbortCondition } from '../../src/conditions/http.conditions.js';
import type { AgainConfig } from '../../src/again/types.js';
...
function makeDeps(overrides?: Partial<ExecutorDependencies>): ExecutorDependencies {
  return {
    ...
    againCondition: defaultAgainCondition,
...
function makeConfig(overrides?: Partial<AgainConfig>): AgainConfig {
  return {
    maxAttempts: 3,
...
describe('AgainExecutor', () => {
  ...
  describe('failure path', () => {
    ...
    it('does not retry when retryIf returns false', async () => {
      ...
      const deps = makeDeps({
        againCondition: { shouldRetry: () => false },
      });
