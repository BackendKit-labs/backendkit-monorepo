/**
 * retry + idempotency integration stress test
 *
 * Prueba tres comportamientos en conjunto:
 *
 *   retry_resilience — retry reintenta el pago hasta 3 veces con backoff exponencial.
 ...
 *     (1) 100% falla → orden 503 (retry agota reintentos)
...
// ── Métricas ────────────────────────────────────────────────────────────────
const retrySuccessRate  = new Rate('retry_resilience_rate');
const replayRate        = new Rate('idempotency_replay_rate');
const lifecycleOk       = new Counter('lifecycle_ok');
const paymentRetries    = new Counter('payment_retries_triggered');
...
export const options = {
  ...
  thresholds: {
    // retry debe superar la tasa de fallo del gateway (60%) con sus reintentos
    retry_resilience_rate:     ['rate>0.65'],
    // >85% de segundos requests deben ser replays
    idempotency_replay_rate: ['rate>0.85'],
...
export function setup() {
  // Alta tasa de fallo para estresar los reintentos de retry
...
// Gateway falla 60% → retry reintenta → P(fallo total) = 0.6^3 = 21.6%
...
export function replayFlow() {
  ...
  // Primera llamada — gateway puede fallar, retry reintenta internamente
  ...
  // Segunda llamada — misma clave, debe ser replay inmediato (no pasa por retry)
...
export function lifecycleFlow(data) {
  ...
  // ── Paso 1: forzar fallo total (retry agota todos sus reintentos) ──────────
...
export function handleSummary(data) {
  const retrySuccess = (data.metrics.retry_resilience_rate?.values?.rate  ?? 0) * 100;
  const replaySucc   = (data.metrics.idempotency_replay_rate?.values?.rate ?? 0) * 100;
  const lifecycle    = data.metrics.lifecycle_ok?.values?.count ?? 0;
  const retriesSeen  = data.metrics.payment_retries_triggered?.values?.count ?? 0;
  const p95          = data.metrics.order_duration_ms?.values?.['p(95)'] ?? 0;

  console.log('\n══════════════════════════════════════════════════════');
  console.log('      retry + idempotency — RESULTADOS');
  console.log('══════════════════════════════════════════════════════');
